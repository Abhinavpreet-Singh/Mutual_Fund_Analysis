...
| `variant_type`       | TEXT    | raw    | Regular or Direct             |
...
Date dimension derived from all reporting dates used in the model.

| Column         | Type    | Source  | Notes                         |
| -------------- | ------- | ------- | ----------------------------- |
| `date_value`   | TEXT    | derived | ISO date string, primary key  |
| `year`         | INTEGER | derived | Calendar year                 |
| `quarter`      | INTEGER | derived | Calendar quarter              |
| `month`        | INTEGER | derived | Calendar month number         |
| `month_name`   | TEXT    | derived | Month abbreviation            |
| `day_of_month` | INTEGER | derived | Day of month                  |
| `day_of_week`  | INTEGER | derived | Monday=1 ... Sunday=7         |
| `day_name`     | TEXT    | derived | Full day name                 |
| `is_weekend`   | INTEGER | derived | 1 for Saturday/Sunday, else 0 |
