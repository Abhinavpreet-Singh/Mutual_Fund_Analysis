...
def build_findings(data: dict[str, pd.DataFrame]) -> list[str]:
    ...
    tier_split = investor_tx[investor_tx["transaction_type"].eq("SIP")].groupby("city_tier", as_index=False).agg(amount_inr=("amount_inr","sum")).sort_values(by="amount_inr", ascending=False)
    top_tier = tier_split.iloc[0]["city_tier"] if not tier_split.empty else "Unknown"

    corr_sample = nav_history.sort_values(["amfi_code", "date"]).copy()
    corr_sample["daily_return"] = corr_sample.groupby("amfi_code")["nav"].pct_change()
    selected_codes = scheme_perf.nlargest(10, "aum_crore")["amfi_code"].tolist()
    corr_matrix = corr_sample[corr_sample["amfi_code"].isin(selected_codes)].pivot(index="date", columns="amfi_code", values="daily_return").corr()
    corr_no_diag = corr_matrix.where(~np.eye(corr_matrix.shape[0], dtype=bool))
    corr_no_diag.index.name = "scheme_a"
    corr_no_diag.columns.name = "scheme_b"
    strongest_corr = (
        corr_no_diag.stack()
        .reset_index()
    )
    strongest_corr.columns = ["scheme_a", "scheme_b", "correlation"]
    strongest_corr = strongest_corr.sort_values(by="correlation", ascending=False)
    if strongest_corr.empty:
        strongest_names = ["n/a", "n/a"]
    else:
        fund_names = _fund_name_map(fund_master)
        top_pair = strongest_corr.iloc[0]
        strongest_names = [
            fund_names.get(cast(int | str, top_pair["scheme_a"]), "n/a"),
            fund_names.get(cast(int | str, top_pair["scheme_b"]), "n/a"),
        ]
...
if __name__ == "__main__":
    ...
    for section in ["nav", "aum_sip", "demographic_geo", "portfolio", "performance"]:
        for path in results[section]:
            print(path)
    print("\nKey findings:")
